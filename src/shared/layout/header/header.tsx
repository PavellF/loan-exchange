...
import { Brand, BrandIconLink } from './header-components';
...
export const AppHeader = (props) => {
  ...
  return (
    <Header style={{ boxShadow: '2px 2px 3px rgba(0,0,0,.1)', background: 'white', zIndex: 1 ,padding: '0 0 0 24px'}}>
      <div className={`Container Row Between Wrap ${classes.HeadFull}`}>
        <Brand />
      ...
      <div className={`Row Between ${classes.HeadShort}`}>
        <BrandIconLink />
        <Menu mode="horizontal" defaultSelectedKeys={[startLinkActive]} style={{ height: '65px', lineHeight: '64px' }}>
          <Menu.Item key={HeaderLink.NOTIFICATIONS}>
            <Link to="/notifications">
          ...
          <Menu.Item key={HeaderLink.USER}>
            <Link to="/account">
              <Avatar style={{ marginLeft: '0.5rem' }}>{firstLetter}</Avatar>
            </Link>
          </Menu.Item>
        </Menu>
      </div>
